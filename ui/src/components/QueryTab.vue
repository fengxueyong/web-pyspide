<template>
  <div class="query-tab">
    ...
    <template v-if="hasQueried">
        ...
        <div v-if="filterOpen" class="filter-panel">
          <div class="filter-group">
            ...
            <div class="filter-chips">
              <button v-for="opt in typeOptions" :key="opt.value" class="chip" :class="{ active: filterType === opt.value }" @click="filterType = opt.value">{{ opt.label }}</button>
            </div>
          </div>
        </div>
      ...
      <div class="content-area" ref="scrollRef">
        <div v-if="filtered.length === 0" class="empty-state">
          ...
          <span class="empty-sub">尝试调整查询或筛选条件</span>
        </div>
        ...
        <div v-else-if="viewMode === 'detail'" class="detail-list">
          <div v-for="r in displayed" :key="r.id || r.res_link" class="detail-card" @click="openLink(r.res_link || r.url)">
            ...
            <div class="card-body">
              <div class="card-row">
                <span class="type-tag" :class="'type-' + (r.res_type || r.type)">{{ typeLabel(r.res_type || r.type) }}</span>
        ...
        <div v-else class="thumb-grid">
          <div v-for="r in displayed" :key="r.id || r.res_link" class="thumb-card" @click="openLink(r.res_link || r.url)">
            <div class="thumb-preview">
              <img v-if="r.res_type === 'image'" :src="r.res_link" :alt="r.res_link" class="thumb-img" @error="onImgError" />
    ...
    <div v-else class="empty-state initial">
      ...
      <span class="empty-title">{{ resources.length === 0 ? '暂无资源数据' : '设置条件后点击查询' }}</span>
      <span v-if="resources.length === 0" class="empty-sub">先在左侧配置并运行抓取任务</span>
    </div>
  </div>
</template>
...
.status-tag.status-ok { color: var(--primary); background: rgba(22, 100, 48, 0.1); }
.status-tag.status-warn { color: #b8860b; background: rgba(184, 134, 11, 0.1); }
.status-tag.status-error { color: var(--destructive); background: rgba(192, 37, 26, 0.1); }
...
.thumb-status.status-ok { background: rgba(22, 100, 48, 0.8); color: white; }
.thumb-status.status-warn { background: rgba(184, 134, 11, 0.8); color: white; }
.thumb-status.status-error { background: rgba(192, 37, 26, 0.8); color: white; }
...
.thumb-name { font-size: 12px; color: var(--foreground); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.thumb-url { font-size: 11px; color: var(--muted-foreground); opacity: 0.6; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
